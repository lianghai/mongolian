...
def writer(builder: otl.CodeBuilder):
    ...
    with builder.File(directory / "classes-letters.fea") as f:
        ...
        for name, letter in script.characters.items():
            if name not in script.categorization.letter:
                continue
            subclasses = []
    ...
    with builder.File(directory / "lookups-joining.fea") as f:
        for joining_form in otl.JOINING_FORM_TAGS:
            with f.Lookup(f"IIa.{joining_form}") as lookup:
                for name in script.categorization.letter:
                    lookup.sub(name).by(mong(name, None, joining_form))

    abstract_variant_to_definite = {}
    condition_to_substitutions = dict[
        str, dict[tuple[str, tuple[Identity, ...]], Identity]
    ]()
    abstract_variant_to_fallback = {}
    affected_variants_and_fvs_to_manual = dict[
        tuple[str, tuple[Identity, ...], str], Identity
    ]()
    for name in script.categorization.letter:
        letter = script.characters[name]
                ...
                if variant.is_definite:
                    ...
                    affected_variants: tuple[Identity, ...] = tuple(
                        ...
                        + [
                            ...
                            if (
                                identity := mong(
                                    name, variant.written_units, joining_form
                                )
                            )
                            != variant_glyph
                    ...
                    for condition in variant.conditions:
                        if condition == "fallback":
                            ...
                            substitutions = condition_to_substitutions.setdefault(
                                condition, {}
                            )
                            substitutions[
                                joining_form_class_name, affected_variants
                            ] = variant_glyph
                    fvs = f"fvs{variant.fvs}"
                    affected_variants_and_fvs_to_manual[
                        joining_form_class_name,
                        affected_variants,
                        fvs,
                    ] = variant_glyph
        ...
        for condition, substitutions in condition_to_substitutions.items():
            with f.Lookup(f"condition.{condition}") as lookup:
                for (
                    joining_form_class_name,
                    affected_variants,
                ), variant in substitutions.items():
    ...
    with builder.File(directory / "lookups-general.fea") as f:
        ...
        with f.Lookup("III.ig.preprocessing.A") as lookup:
            for name in script.categorization.letter.vowel.masculine:
                for joining_form in otl.JOINING_FORM_TAGS:
                    abstract_variant = mong(name, None, joining_form)
        ...
        with f.Lookup("III.ig.preprocessing.B").flag(
            mark_filtering_set="@signal.masculine"
        ) as lookup:
            for name in [
                *script.categorization.letter.vowel.neuter,
                *script.categorization.letter.consonant,
            ]:
                for joining_form in otl.JOINING_FORM_TAGS:
                    abstract_variant = mong(name, None, joining_form)
                    lookup.prefix("masculine").sub(abstract_variant).by(
                        abstract_variant, "masculine"
                    )

        with f.Lookup("III.ig.preprocessing.C") as lookup:
            for name in [
                *script.categorization.letter.vowel.masculine,
                *script.categorization.letter.vowel.neuter,
                *script.categorization.letter.consonant,
            ]:
                if name == "g":
        ...
        with f.Lookup("III.o_u_oe_ue.marked").flag("IgnoreMarks") as lookup:
            lookup.prefix("@consonant.init").sub(["@o", "@u", "@oe", "@ue"]).chain(
                conditions("marked")
            )
        ...
        with f.Lookup("III.h_g.onset_and_devsger_and_gender.A").flag(
            "IgnoreMarks"
        ) as lookup:
            lookup.sub(["@h", "@g"]).chain(conditions("masculine_onset")).suffix(
                "@vowel.masculine"
            )
            lookup.sub(["@h", "@g"]).chain(conditions("feminine")).suffix(
                ["@vowel.feminine", "@vowel.neuter"]
            )
            lookup.prefix("@vowel.masculine").sub("@g").chain(
                conditions("masculine_devsger")
            )
            lookup.prefix("@vowel.feminine").sub("@g").chain(conditions("feminine"))

        with f.Lookup("III.h_g.onset_and_devsger_and_gender.B").flag(
            mark_filtering_set="@signal.masculine"
        ) as lookup:
            lookup.ignore().sub("@g").suffix("@vowel")
            lookup.ignore().sub("@g").suffix("masculine", "@vowel")
            lookup.prefix("@i").sub("@g").chain(conditions("masculine_devsger")).suffix(
                "masculine"
            )
        ...
        with f.Lookup("III.ig.postprocessing") as lookup:
            name = "g"
            for joining_form, variants in script.characters[
                name
            ].variants_by_joining_form.items():
                for variant in variants:
        ...
        with f.Lookup("III.y.dictionary_particle").flag("IgnoreMarks") as lookup:
            ...
            lookup.prefix(["mvs.effective", "nnbsp.effective"], "i.I.init").sub(
                "y.Y.medi"
            ).chain(conditions(c)).suffix(
                ["a.A.medi", "e.A.medi"], ["n.A.fina", "r.R.fina"]
            )
            lookup.sub(["mvs", "nnbsp"], "y.Y.init").chain(
                _effective, conditions(c)
            ).suffix("i.I.fina")
            lookup.sub(["mvs", "nnbsp"], "y.Y.init").chain(
                _effective, conditions(c)
            ).suffix("i.I.medi", "n.A.fina")
        ...
        with f.Lookup("III.i.devsger").flag("IgnoreMarks") as lookup:
            lookup.glyph_class(
                ...
                [
                    mong(name, v.written_units, joining_form)
                    for name in script.categorization.letter.vowel
                    for joining_form, variants in script.characters[
                        name
                    ].variants_by_joining_form.items()
                    for v in variants
        ...
        with f.Lookup("III.o_u_oe_ue.post_bowed").flag("IgnoreMarks") as lookup:
            lookup.glyph_class(
                context_class_name := "@B_P_G_Gx_F_K",
                [
                    mong(name, v.written_units, joining_form)
                    for name in script.categorization.letter
                    for joining_form, variants in script.characters[
                    ...
                    for v in variants
                    if (not v.is_manual)
                    and v.written_units[-1] in ["B", "P", "G", "Gx", "F", "K"]
                ],
            )
            lookup.glyph_class(
                target_class_name := "@o_u_oe_ue.U",
                [
                    mong(name, v.written_units, joining_form)
            ...
            lookup.prefix(context_class_name).sub(target_class_name).chain(
        ...
        with f.Lookup("III.fvs") as lookup:
            ...
            with f.Lookup("_.manual") as _manual:
                for (
                    ...
                    affected_variants,
                    fvs,
                ), manual in affected_variants_and_fvs_to_manual.items():
                ...
                for variant in variants:
                    ...
                    written_unit_parts = tuple(
                        ...
                        for written_unit, sliced_joining_form in zip(
                            ...
                            slice_joining_form(
                                joining_form, len(variant.written_units)
                            ),
                        )
                    )
                    pua_parts = tuple(
                        f":pua{script.written_units[written_unit].variant_by_joining_form[sliced_joining_form].menksoft_pua:04X}"
                        for written_unit, sliced_joining_form in zip(
                            variant.written_units,
                            slice_joining_form(
                                joining_form, len(variant.written_units)
    ...
    with builder.File(path) as f:
        ...
        with f.Table("GDEF") as table:
            table.glyph_class_definition(
                ...
                marks=[  # Somehow cannot restore advance in dist.
                    *format_control.joining_control,
                    *format_control.fvs,
                    "nil",
                    "masculine",
                ],
        ...
        f.language_system(script.otl_tags()[0])
